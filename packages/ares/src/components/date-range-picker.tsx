...
import { ChevronUpIcon, ChevronDownIcon } from '@remis/icons/chevron';
...
const getDateAdjustedForTimezone = (dateInput: Date | string): Date => {
  if (typeof dateInput === 'string') {
    // Handle ISO date strings (YYYY-MM-DD) or other date strings
    ...
    if (isNaN(date.getTime())) {
      ...
      if (parts.length === 3 && parts.every((part) => !isNaN(part))) {
        // Month is 0-indexed, so subtract 1 from the month part
...
export const DateRangePicker: FC<DateRangePickerProps> & {
  ...
  const PresetButton = ({
    ...
    <Button
      className={cn(
        'w-full justify-start text-left h-8 px-3 py-1.5 text-sm font-normal',
        ...
        <div className="flex py-3">
          <div className="flex">
            <div className="flex flex-col">
              <div className="flex flex-col lg:flex-row gap-3 px-4 justify-end items-center lg:items-start pb-4 lg:pb-0">
                {showCompare && (
          ...
          {!isSmallScreen && (
            <div className="flex flex-col items-end gap-1 pr-4 pl-6 pb-6">
...
DateRangePicker.filePath = 'packages/ares/src/components/date-range-picker.tsx';
