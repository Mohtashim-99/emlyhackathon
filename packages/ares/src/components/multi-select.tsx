...
interface MultiSelectProps
  extends
    Omit<
      ...
      'animationConfig' | 'defaultValue' | 'value'
    >,
   ...
   * If options have `id` properties, returns `{ id: number; value: string }[]`.
   * Otherwise, returns `string[]` for backward compatibility.
   */
  // eslint-disable-next-line no-unused-vars
  onValueChange: (values: string[] | { id: number; value: string }[]) => void;

  /** The default selected values when the component mounts. */
  defaultValue?: string[] | { id: number; value: string }[];

  /**
   * Controlled value for the component.
   * Can be either `string[]` or `{ id: number; value: string }[]`.
   * If options have `id` properties, use the object format.
   */
...
export const MultiSelect = React.forwardRef<MultiSelectRef, MultiSelectProps>(
  (
    ...
    // Helper to normalize defaultValue and value to string[]
    const normalizeToStringArray = React.useCallback(
      (val: string[] | { id: number; value: string }[] | undefined): string[] => {
        if (!val) return [];
        if (Array.isArray(val) && val.length > 0) {
          if (typeof val[0] === 'string') {
            return val as string[];
          }
          return (val as { id: number; value: string }[]).map((item) => item.value);
        }
        return [];
      },
      [],
    );
    const initialValue = normalizeToStringArray(value ?? defaultValue);
    ...
    const prevDefaultValueRef = React.useRef<string[]>(normalizeToStringArray(defaultValue));
    ...
    // Check if options have id properties
    const optionsHaveIds = React.useMemo(() => {
      const allOptions = getAllOptions();
      return allOptions.length > 0 && allOptions[0] && allOptions[0].id !== undefined;
    }, [getAllOptions]);

    // Transform selected values to the appropriate format before calling onValueChange
    const transformAndCallOnValueChange = React.useCallback(
      (values: string[]) => {
        if (optionsHaveIds) {
          const allOptions = getAllOptions();
          const transformed = values
            ...
            .filter((item): item is { id: number; value: string } => item !== null);
          onValueChange(transformed);
        } else {
          onValueChange(values);
        }
      },
      [optionsHaveIds, getAllOptions, onValueChange],
    );

    const resetToDefault = React.useCallback(() => {
      const defaultVal = normalizeToStringArray(defaultValue);
      setSelectedValues(defaultVal);
      setIsPopoverOpen(false);
      setSearchValue('');
      transformAndCallOnValueChange(defaultVal);
    }, [defaultValue, normalizeToStringArray, transformAndCallOnValueChange]);
    ...
    React.useImperativeHandle(
      ...
      () => ({
        ...
        setSelectedValues: (values: string[]) => {
          ...
          transformAndCallOnValueChange(values);
        },
        clear: () => {
          setSelectedValues([]);
          transformAndCallOnValueChange([]);
        },
        focus: () => {
      ...
      [resetToDefault, selectedValues, transformAndCallOnValueChange],
    );
    ...
    React.useEffect(() => {
      if (value !== undefined) {
        const normalizedValue = normalizeToStringArray(value);
        const prevValue = prevValueRef.current;
        if (!prevValue || !arraysEqual(prevValue, normalizedValue)) {
          setSelectedValues(normalizedValue);
          prevValueRef.current = normalizedValue;
        }
      }
    }, [value, normalizeToStringArray, arraysEqual]);
    ...
    const handleInputKeyDown = (event: React.KeyboardEvent<HTMLInputElement>) => {
      if (event.key === 'Enter') {
        ...
        transformAndCallOnValueChange(newSelectedValues);
      }
    };
    ...
    const toggleAll = () => {
      ...
      if (selectedValues.length === allOptions.length) {
        ...
        transformAndCallOnValueChange(allValues);
      }
    ...
    React.useEffect(() => {
      ...
      const normalizedDefault = normalizeToStringArray(defaultValue);
      const prevDefaultValue = prevDefaultValueRef.current;
      if (!arraysEqual(prevDefaultValue, normalizedDefault)) {
        if (!arraysEqual(selectedValues, normalizedDefault)) {
          setSelectedValues(normalizedDefault);
        }
        prevDefaultValueRef.current = [...normalizedDefault];
      }
    }, [
      defaultValue,
      selectedValues,
